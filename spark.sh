...
function usage {
  ...
  echo -e "\t--keep-tmp\t\tKeep the temporary installation and log directories of hail and spark after the job finishes (default: false)."
  echo -e "\t-h,--help\t\tPrint this help message"
}

options=$(getopt -o ihXs: --long "install-hail help no-use-srun script: conda-env: conda-module: hail-version: spark-module: spark-version: spark-interface: ipoib-domain: master-cores: master-mem: scratch-dir: conda-init: install-s3-connector keep-tmp" -- "$@")
[ $? -eq 0 ] || {
  usage
